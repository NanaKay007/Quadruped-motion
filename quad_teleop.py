...
def main():
    ...
    env.commands = torch.tensor([[lin_x, lin_y, ang_z]]).to("mps").repeat(num_envs, 1)
    iter = 0
    ...
    with torch.no_grad():
        while not stop:
            actions = policy(obs)
            env.commands = torch.tensor([[lin_x, lin_y, ang_z]], dtype=torch.float).to("mps").repeat(num_envs, 1)
