...
class QuadrupedNavigatorEnv(VecEnv):
    def __init__(
        self,
        num_envs,
        controler_policy,
        ...
        self.controler_policy = controler_policy
        ...
        self.target =self.scene.add_entity(
            gs.morphs.Box(
                size=(0.5, 0.5, 0.5),
                pos=(TARGET_BOX_DISTANCE, 0.0, 0.25),
    ...
    def reset_indx(self, envs_indx):
        ...
        self.robot.set_qpos(
            torch.cat(
                [
                    self.base_pos[envs_indx],
                    self.base_quat[envs_indx],
                    self.dof_pos[envs_indx],
                ],
                dim=-1,
            ),
            envs_idx=envs_indx,
    ...
    def step(self, navigator_actions, is_train=True):
        ...
        loco_obs = torch.cat(
            [
                ...
                navigator_actions
                * self.commands_scale,  # 4 (lin_vel_x, lin_vel_y,ang_vel, height)
        ...
        # Get joint-level actions from locomotion policy (no grad — inference only)
        with torch.no_grad():
            self.actions = self.controler_policy(loco_obs)
    ...
    def run_locomotion_policy(self, obs):
        with torch.no_grad():
            target_dof_pos = self.controler_policy(obs)
